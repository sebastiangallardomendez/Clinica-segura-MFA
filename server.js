...
app.post('/login', (req, res) => {
    ...
    if (user.password !== password) {
        user.loginAttempts += 1; // Fallo de contraseña
        
        if (user.loginAttempts >= 3) {
            user.isLocked = true;
            
            // Alerta de Contraseña Fallida
            const mailOptions = {
                from: '"Seguridad Clínica" <tu_correo@gmail.com>',
                to: email,
                subject: 'ALERTA: Bloqueo por contraseña incorrecta',
                text: `Hola ${user.nombre}, se han detectado 3 intentos fallidos de contraseña. Tu cuenta ha sido bloqueada.`
            };
            transporter.sendMail(mailOptions, (err) => { if(err) console.log(err); });

            return res.status(403).json({ message: 'Has excedido los intentos de contraseña. CUENTA BLOQUEADA.' });
        }
    ...
    user.loginAttempts = 0; // Reseteamos fallos de pass
    user.mfaAttempts = 0;   // Reseteamos fallos de MFA (NUEVO)
